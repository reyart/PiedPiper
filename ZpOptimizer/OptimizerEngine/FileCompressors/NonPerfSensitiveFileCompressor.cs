    ...
    public class NonPerfSensitiveFileCompressor : FileCompressor {
        ...
        public override void Execute()
        {
            ...
            { // Lower compression if it compresses poorly
                ...
                currentDirLogger.WriteLine(sizeBefore + "," + fileToCompress.GetSizeOnDisk() + "," + Math.Round(compRatio, 2) + ",XPRESS16K,Downgraded");
            }
            else
            {
                // Stick with the original compression and write to the logger if neither applies
                currentDirLogger.WriteLine(sizeBefore + "," + fileToCompress.GetSizeOnDisk() + "," + Math.Round(compRatio, 2) + ",LZX");
            }
